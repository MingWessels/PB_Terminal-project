...
class Baby {
  ...
  getStatus() {
    let status;
    if (this.energy >= 50 && this.fun >= 50 && this.tiredness <= 50 && this.diaperLevel <= 50) {
      console.log(`
      __\\|/__
     /       \\
    /  o   o  \\
  (|   __^__   |)
    \\  \\___/  /
     \\_______/
`);
      console.log(chalk.green(`Du hast gewonnen! ${this.name} ist glücklich und lacht!\n`));
      status = 'gewonnen';
    } else if (
      this.energy <= 0 || this.fun <= 0 || this.tiredness > 50 || this.diaperLevel > 50) {
      console.log(`
      __\\|/__
     /       \\
    /  >   <  \\
...
`);
  ...
  (|     ^     |)
    \\  \ ---   /
     \\_______/
`);
      console.log(chalk.yellow(`${this.name} ist noch nicht glücklich. Versuche es weiter!\n`));
      status = 'weitermachen';
    }
    return status;
  }
  ...
  showStatus() {
    ...
    console.log(chalk.green(`Du brauchst mindestens 50 Energiepunkte und 50 Spaßpunkte, um ${this.name} zum lachen zu bringen.`));
    console.log(chalk.red(`Aber Vorsicht, die Müdigkeits- und Windelpunkte von ${this.name} dürfen 50 nicht überschreiten!\n`));
  }
}
...
while (true) {
  ...
  const choiceSelection = readlineSync.keyInYN('Möchtest du noch ein Spiel spielen?');
